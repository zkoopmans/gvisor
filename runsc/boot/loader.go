...
func (l *Loader) signalProcess(cid string, tgid kernel.ThreadID, signo int32) error {
	...
	if err == nil {
		...
		return l.k.SendExternalSignalTg(execTG, &arch.SignalInfo{Signo: signo})
	}
	...
	return l.k.SendExternalSignalTg(tg, &arch.SignalInfo{Signo: signo})
}
...
func (l *Loader) signalForegrondProcessGroup(cid string, tgid kernel.ThreadID, signo int32) error {
	...
	for _, tg := range l.k.TaskSet().Root.ThreadGroups() {
		...
		if err := l.k.SendExternalSignalTg(tg, &arch.SignalInfo{Signo: signo}); err != nil {
			lastErr = err
		}
	}
	return lastErr
